...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ADDRESS   0x24<<1 // in case PCF8574 with A1,A0 short
...
void send_byte2PCF8574(uint8_t data);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if(led_data == 256) led_data = 1;
	  else = led_data = led_data * 2;
		  send_byte2PCF8574(led_data); HAL_Delay(500);
		  	  send_byte2PCF8574(0x00); HAL_Delay(500);
    /* USER CODE END WHILE */
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
