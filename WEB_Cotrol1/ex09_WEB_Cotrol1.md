...
Node.js로 작성한 웹페이지를 통해 타겟보드의 LED를 제어하고, 타겟보드에 연결된 가변저항의 ADC 값을 실시간으로 웹페이지에 디스플레이 해보자.

#### 1. 펌웨어 작성

시리얼통신으로 문자 '1'를 수신하면 On Board LED(Green)를 켜고, 문자 '0'를 수신하면 On Board LED(Green)를 Off하는 코드를 작성하여 타겟보드에 업로드해 둔다.
